...
def _resolve_device(backend):
    ...
    if backend == "rocm":
        if not _is_rocm_build():
            raise RuntimeError(
                "ROCm was selected, but this PyTorch build has no HIP/ROCm support. "
                "Install a ROCm build of PyTorch and restart ComfyUI."
            )
        if not torch.cuda.is_available():
            raise RuntimeError(
                "ROCm was selected, but PyTorch cannot access an AMD GPU. "
                "Check the ROCm driver, device permissions, and PyTorch installation."
            )
        # PyTorch intentionally uses the 'cuda' device type for ROCm/HIP GPUs.
        return torch.device("cuda")

    if backend == "cuda":
        # Preserve the node's previous behavior: fall back to CPU when the
        # requested CUDA-compatible device is unavailable.
...
def load_model(name, device, precision):
    cache_key = f"{name}::{device}::{_backend_label(device)}::{precision}"
    ...
    print(f"  Params: {sum(p.numel() for p in model.parameters()):,} | "
          ...
          f"Backend: {_backend_label(device)} | Precision: {precision}")
    return model
...
class MinimaxH3LatentUpscaler3D(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.Combo.Input("device", options=["cuda", "rocm", "cpu"], default="cuda",
                               tooltip="Execution backend. ROCm requires a HIP-enabled PyTorch build; internally it uses PyTorch's cuda device API."),
    ...
    def execute(cls, latent: dict, model_name: str, mode: UpscaleConfig,
        ...
        # torch.cuda also owns the allocator/cache API on ROCm builds.
        if dev.type == "cuda":
